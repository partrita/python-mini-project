...
console_handler.setLevel(
    logging.DEBUG
)  # Imposta il livello a INFO o altro a tuo piacere
...
class Ball:
    ...
    def update_pos(self) -> None:
        if (
            self.center_y - self.radius <= 0
            or self.center_y + self.radius >= self.display_high
        ):
    ...
    def draw(self, display: pygame.Surface) -> None:
        ...
        pygame.draw.circle(
            self.surface, self.color, (self.center_x, self.center_y), self.radius
        )
...
class PongGame:
    def __init__(
        ...
        self._fps = fps
        self._BALL_SPEED_X_DFLT = (self._game_field.disp_w) / (
            self._max_ball_time_4_travel * self._fps
        )
        self._BALL_SPEED_Y_DFLT = (self._game_field.disp_h) / (
            self._max_ball_time_4_travel * self._fps
        )
        self._CPU_SPEED_DFLT = (self._game_field.disp_h) / (
            self._max_gamer_time_4_travel * self._fps
        )
        self._cpu_speed_increment = 1
        self._ball_speed_increment = 0.5
        self._player_speed = (self._game_field.disp_h) / (
            self._max_gamer_time_4_travel * self._fps
        )
    ...
    def _check_collision(self):
        ...
        if (
            ball_coord["right"] > self._game_field.disp_w / 2
            and ball_coord["right"] >= self._player.get_borders()["left"]
        ):
            if (
                ball_coord["top"] <= self._player.get_borders()["bott"]
                and ball_coord["bott"] >= self._player.get_borders()["top"]
            ):
                self._ball.invert_move(invert_x=True)

        if (
            ball_coord["left"] < self._game_field.disp_w / 2
            and ball_coord["left"] <= self._computer.get_borders()["right"]
        ):
            if (
                ball_coord["top"] <= self._computer.get_borders()["bott"]
                and ball_coord["bott"] >= self._computer.get_borders()["top"]
            ):
                self._ball.invert_move(invert_x=True)
    ...
    def _update_game_speed(self):
        ...
        if score_diff > 0 and score_diff > self._stat["last_diff"]:
            self._ball.set_speed(
                abs(self._ball.get_speed()[0]) + self._cpu_speed_increment,
                abs(self._ball.get_speed()[1]) + self._cpu_speed_increment,
            )
            self._computer.set_speed(self._cpu_speed + self._cpu_speed_increment)
    ...
    def _move_computer(self):
        if self._ball.get_borders()["right"] < self._game_field.disp_w / 2:
            if (
                self._computer.get_borders()["center_y"]
                < self._ball.get_borders()["center_y"]
            ):
    ...
    def _write_score(self):
        ...
        img = font.render(
            f"{self._stat['cpu_score']}    {self._stat['player_score']}",
            True,
            self._game_field.line_color,
        )
        scor_str_len = (
            len(str(f"{self._stat['cpu_score']}    {self._stat['player_score']}"))
            * font_size
        )
    ...
    def _write_win(self, winner_name: str):
        font_size = 30
        font = self._pong_pygame.font.SysFont("freemono", font_size)
        img = font.render(f"{winner_name} wins!", True, self._game_field.line_color)
        win_name_str = len(str(f"{winner_name} wins!")) * font_size
        self._game_field.screen.blit(
    ...
    def _update_events(self):
        ...
        for event in self._pong_pygame.event.get():
            ...
            if (
                event.type == self._pong_pygame.KEYDOWN
                or event.type == self._pong_pygame.KEYUP
            ):
                start_speed_x = get_random(1) * (
                    self._BALL_SPEED_X_DFLT
                    + (self._cpu_speed_increment * self._stat["level"])
                )
                start_speed_y = self._BALL_SPEED_Y_DFLT + (
                    self._cpu_speed_increment * self._stat["level"]
                )
...
def main():
    ...
    parser.add_argument(
        "-dw", "--width", type=int, default=320, help="Width of the display (dflt 320)"
    )
    parser.add_argument(
        "-dh",
        "--height",
        type=int,
        default=240,
        help="Height of the display (dflt 240)",
    )
    parser.add_argument(
        "-n", "--name", type=str, default="player 1", help="Player name"
    )
    parser.add_argument(
        "-c",
        "--color",
        type=str,
        default="lightgrey",
        help="Game color (dflt light grey)",
    )
    parser.add_argument("--fps", type=int, default=120, help="Framerate (dflt 120)")
    parser.add_argument(
        "--max_score", type=int, default=10, help="Max score to win (dflt 10)"
    )
    ...
    game_field = GameField(
        screen_width, screen_height, "black", rgb_colors[color], "Pong"
    )
    ball = Ball(
        screen_width / 2,
        screen_height / 2,
